...
class RandomBurstSource(AbstractSource):
    """
    Provides static input in burst patterns with stochastic firing.
    Modified to randomly select neurons at each timestep.
    """
    def __init__(self, input_values_e, firing_prob=0.4, burst_length=50, gap_length=150,
                 input_values_i=None, connection_density_e=1.0, 
                 connection_density_i=0.0, eta_stdp=0.0):
        ...
        # CHANGED: Now we need N neurons, not just 1
        self.N_e = None  # Will be set in generate_connection_e
        self.N_i = None  # Will be set in generate_connection_i
        self.N = None    # Total input neurons
        self.t = 0
        
    def next(self):
        """Burst pattern with stochastic firing to random neurons"""
        if self.N is None:
            raise RuntimeError("generate_connection_e/i must be called before next()")
        ...
        if phase <= self.burst_length:
            # In burst period
            if np.random.rand() < self.firing_prob:
                # Fire! But to random subset
                # E neurons: randomly select connection_density_e fraction
                if self.N_e is not None:
                    selected_e = np.random.rand(self.N_e) < self.connection_density_e
                    output[:self.N_e] = selected_e.astype(float)
                
                # I neurons: randomly select connection_density_i fraction  
                if self.N_i is not None and self.connection_density_i > 0:
                    selected_i = np.random.rand(self.N_i) < self.connection_density_i
                    output[self.N_e:] = selected_i.astype(float)
    ...
    def generate_connection_e(self, N_e):
        """Create one-to-one connections for dynamic selection"""
        self.N_e = N_e
        ...
        # CHANGED: One-to-one mapping so we can control each neuron
        W = np.zeros((N_e, self.N))
        for i in range(N_e):
            W[i, i] = values[i]
    ...
    def generate_connection_i(self, N_i):
        ...
        if self.input_values_i is not None and self.connection_density_i > 0:
            ...
            # One-to-one for I neurons starting after E neurons
            for i in range(N_i):
                W[i, self.N_e + i] = values[i]
